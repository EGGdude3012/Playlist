...
public class PlaylistServlet extends HttpServlet {
    public void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException 
    {
        ...
        String listaInicial = request.getParameter("secreto");
        String[] listaMedia = listaInicial.split(",");
        int listaFinal []= new int[listaMedia.length];
        for (int i=0; i<listaMedia.length; i++) {
            listaFinal[i] = Integer.parseInt(listaMedia[i]);
        }
        
        List<MusicLibrary> playlist = new ArrayList<MusicLibrary>();
                
        if (request.getParameter("orden").equals("Cola")) {
            
        
            for (int i = 0; i < listaFinal.length; i++) {
                for (MusicLibrary cancion : canciones) {
                    if (listaFinal[i] == cancion.getId()) {
        ...
        if (request.getParameter("orden").equals("Pila")) {
            for (int i = 0; i < listaFinal.length; i++) {
                for (MusicLibrary cancion : canciones) {
                    if (listaFinal[i] == cancion.getId()) {
                    playlist.add(cancion);
        ...
        if(request.getParameter("orden").equals("Aleatorio")) {
            for (int i = 0; i < listaFinal.length; i++) {
                for (MusicLibrary cancion : canciones) {
                    if (listaFinal[i] == cancion.getId()) {
                    playlist.add(cancion);
